...
pedestal = 30
n_janelamento = [7,9,11,13,15,17,19,21]
...
for d in range(len(n_janelamento)):
    for f in range(len(ocupacao)):
        ...
        num_linhas = len(dados_Ocupacao) - (n_janelamento[d]-1)
        # print("Número de linhas da matriz", num_linhas)

        # Inicializar a matriz para armazenar os blocos de amostras, com num_linhas linhas e n_janelamento[d] colunas
        matriz_Ocupacao_Amostras = np.zeros((num_linhas, n_janelamento[d]))

        # Preencher a matriz com os blocos de amostras
        for i in range(num_linhas):
            inicio = i #começa no ponto atual
            fim = i + n_janelamento[d] #vai até o ponto atual + n_janelamento[d]
        ...
        indice_central = n_janelamento[d] // 2
        ...
        # preenche as primeiras n_janelamento[d] casas(tanto linhas como colunas) da matriz A com a matriz de covariancia do ruido
        ...
        B = np.zeros((n_janelamento[d]+3, 1))
        B[n_janelamento[d]]=1
        ...
        for i in range(len(matriz_Ocupacao_Amostras)): #linhas
            ...
            for j in range(n_janelamento[d]): #colunas
        ...
        for indice, linha in enumerate(linhas):
            if linha.startswith(str(n_janelamento[d]) + ' ' + str(ocupacao[f]) +  ' '):
                linha_existente = indice
                break

        # Se a linha existir, sobrescrever com os novos valores
        if linha_existente is not None:
            linhas[linha_existente] = f"{n_janelamento[d]} {ocupacao[f]} {mediaCadaFold} {desvioPadraoCadaFold} {mediaDaMedia} {desvioPadraoDoDesvioPadrao}\n"
            ...
            print(f"Dados atualizados para janelamento {n_janelamento[d]} e ocupacao {ocupacao[f]}")
        else:
            ...
            with open(caminho_arquivo_mediaDaMedia, 'a') as arquivo:
                arquivo.write(f"{n_janelamento[d]} {ocupacao[f]} {mediaCadaFold} {desvioPadraoCadaFold} {mediaDaMedia} {desvioPadraoDoDesvioPadrao}\n")
            print(f"Dados adicionados para janelamento {n_janelamento[d]} e ocupacao {ocupacao[f]}")
