...
for f in range(len(ocupacao)):
    ...
    for d in range(len(n_janelamento)):
        ...
        plt.hist(erroEstimacaoKFold, bins=150, alpha=0.7, histtype='step', linewidth=2, label=f"Janelamento: {n_janelamento[d]} ($\mu$= {mediaKfold[-1]:.3f}, $\sigma$={desvioPadraoKfold[-1]:.3f})")
